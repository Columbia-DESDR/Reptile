...
// TODO: alphabet is by convention using these letters, update it to use abcd... instead?
...
const renderDataViews = () => {
  ...
  hierarchy.toReversed().forEach((level, idx) => {
    const thisLetter = alphabet.slice(0, hierarchy.length).toReversed()[idx];
    ...
    const barChart = new BarChart(`#${thisLetter}_3`, {
      color: [WHITE, colorFarmers],
    });
    ...
    const heatMap = new HeatMap(`#${thisLetter}_1`, {
      color: [WHITE, colorFarmers],
    });
  ...
  if (satellite_data.length > 0) {
    satelliteHeatMap = new CountrySatelliteHeatMap("#svg-satellite", {
      color: [WHITE, colorSatellite],
    });
