from flask import request, Response, json, render_template, Flask
import db
import readcsv
...
# == Rendering HTML template with specific variables ==
...
def api_heatmapdata():
    ...
    if (filename, tuple(hiearchy), category) in data_cache:
        ...
        if category is not None:
            d.categorize_attribute(category)
        data_cache[(filename, tuple(hiearchy), category)] = d
    result = d.get_heatmap_data(hierchy_values, categorical, numerical, aggragation)
    # print(result)
...
explanations = [
    ...
    {"name": "explan", "type": "radio", "display": "The rank is missing", "value": "3"},
    {
        "name": "explan",
        "type": "radio",
        "display": "Modify the start value",
        "value": "4",
    },
    {
        "name": "explan",
        "type": "radio",
        "display": "Modify the end value",
        "value": "5",
    },
    {
        "name": "explan",
        "type": "radio",
        "display": "Exchange the start and end value",
        "value": "6",
    },
    {
        "name": "explan",
        "type": "radio",
        "display": "Exchange the ranks of two years",
        "value": "7",
    },
...
solutions = [
    {
        "name": "solution",
        "type": "dropdown",
        "display": "Correct the rank of village: ",
    ...
    },
    {
        "name": "solution",
        "type": "text",
        "display": "Correct the name of village: ",
        "val": 1,
    },
]
...
def api_explan():
    ...
    if request.json["visual"]["filename"] == filename1:
        ...
        if request.json["comp"]["rank"] is None:
            ...
            if "mean" in request.json["des"]:
                ...
                checked["display"] = (
                    "Mean rank should be" + ": " + str(request.json["des"]["mean"])
                )
                ...
                checked["checked"] = "true"
                checked["display"] = (
                    "Exchange the ranks of two years"
                    + ": "
                    + str(request.json["comp"]["year"])
                    + " and "
                    + str(request.json["des"]["year"])
                )
                data["explan"].append(checked)
    ...
    if (
        len(request.json["visual"]["hiearchy"])
        - len(request.json["visual"]["hierchy_values"])
        > 1
    ):
    ...
    data["explan"].append(
        {
            "name": "submit_button",
            "type": "button",
            "display": "Choose the Explanation",
            "onclick": "Solution()",
        }
    )
    js = json.dumps(data)
...
def api_explan2():
    ...
    if request.json["level"] == "region":
        data = readcsv.getRegionExplanation(
            request.json["value"],
            request.json["year"],
            request.json["aggOriginal"],
            request.json["complained_agg"],
            request.json["com_too_small"],
        )
    else:
        data = readcsv.getDistrictExplanation(
            request.json["value"],
            request.json["year"],
            request.json["aggOriginal"],
            request.json["complained_agg"],
...
def api_sol():
    ...
    data = d.complaint(
        hierchy_values, categorical, numerical, aggragation, year, de_mean
    )
    # print(data)
    js = json.dumps(data)
    # # print(js)
