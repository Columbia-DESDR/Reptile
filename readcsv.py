...
class HierachyData:
    ...
    def get_summary2(self):
        self.summary = (
            self.data.reset_index()[self.hiearchy[0:4]]
            .groupby(self.hiearchy[0:3])
            .agg(["unique"])
    ...
    def get_summary(self):
        self.summary = (
            self.data.reset_index()[self.hiearchy[0:2]]
            .groupby(self.hiearchy[0])
            .agg(["unique"])
            .to_json(orient="index")
        )
        return self.summary
    ...
    def get_unique(self, hierchy_values):
        if len(hierchy_values) == 0:
            return (
                self.data.reset_index()[self.hiearchy[len(hierchy_values)]]
                .unique()
                .tolist()
            )
        return (
            self.data.loc[tuple(hierchy_values)]
            .reset_index()[self.hiearchy[len(hierchy_values)]]
            .unique()
            .tolist()
        )
    ...
    def get_data(self, hierchy_values):
        return (
            self.data.loc[tuple(hierchy_values)].reset_index().to_json(orient="records")
        )
    ...
    def get_data2(self, set_values):
        ...
        for value in set_values:
            try:
                ...
                pass
        result["rank"] = result.groupby("District")["value"].rank(
    ...
    def get_heatmap_data(self, hierchy_values, categorical, numerical, aggragation):
        return self.get_heatmap(
            hierchy_values, categorical, numerical, aggragation
        ).to_json(orient="records")
        # return self.data.loc[tuple(hierchy_values)].groupby(categorical).agg(aggragation).reset_index().to_json(orient='records')

    def get_heatmap_withtotal(
        self, hierchy_values, categorical, numerical, aggragation
    ):
        ...
        each["rank"] = (
            (each["mean"] * each["count"])
            .groupby(categorical[0])
    ...
    def get_heatmap_withoutagg(
        ...
        aggregation = ["mean", "std", "count"]
        each = (
            self.data.loc[tuple(hierchy_values)].groupby(categorical).agg(aggregation)
        )
        each.columns = ["mean", "std", "count"]
        each["rank"] = (
            (each["mean"] / each["count"]).groupby(categorical[0]).rank(ascending=True)
        )
        return each.reset_index()

    def complaint(
        self, hierchy_values, categorical, numerical, aggragation, year, de_mean
    ):
        df_all = self.get_heatmap_withoutagg(
            hierchy_values, categorical, numerical, aggragation
        )
...
def getRegionExplanation(region, year, aggOriginal, complained_agg, com_too_small):
    ...
    aggg = RegionData.groupby(["region", "year", "district"])["rank"].agg(
        ["count", "std", "mean"]
    )
    aggg = aggg.fillna(0)
    explanations = []
    ...
    for index, row in selected.iterrows():
        ...
        if (
            not com_too_small
            and complained_agg == "mean"
            and aggOld["count"] == 0
            and (aggNew["count"] < 1 or aggNew["mean"] < 1)
        ):
            continue
        aggAfterRepair = add(remove(aggOriginal, aggOld), aggNew)
        if (
            com_too_small
            and aggAfterRepair[complained_agg] > aggOriginal[complained_agg] + 0.01
        ) or (
            not com_too_small
            and aggAfterRepair[complained_agg] < aggOriginal[complained_agg] - 0.01
        ):
...
def getDistrictExplanation(district, year, aggOriginal, complained_agg, com_too_small):
    ...
    aggg = RegionData.groupby(["district", "year", "village"])["rank"].agg(
        ["count", "std", "mean"]
    )
    aggg = aggg.fillna(0)
    explanations = []
...
def add(agg1, agg2):
    ...
    if agg3["count"] == 0:
        ...
        agg3["std"] = 0
        return agg3
    agg3["mean"] = (agg1["count"] * agg1["mean"] + agg2["count"] * agg2["mean"]) / agg3[
        "count"
    ]
    if agg3["count"] == 1:
        agg3["std"] = 0
        return agg3
    var = (
        (agg1["count"] - 1) * (agg1["std"] ** 2)
        + (agg2["count"] - 1) * (agg2["std"] ** 2)
        + agg1["count"] * ((agg1["mean"] - agg3["mean"]) ** 2)
        + agg2["count"] * ((agg2["mean"] - agg3["mean"]) ** 2)
    ) / (agg3["count"] - 1)
    if var < 0:
        var = 0
    agg3["std"] = var ** (0.5)
